...
FILE_PATHS = {
    ...
    "PHASE1_DATA":     BASE_DIR.parent / "phase1_test" / "phase1_data_soil3.json",
    ...
    "PHASE2_RESPONSE_PREDICTIONS": Path(
        "/root/water/wyc_training/phase3_response_predictions.json"
    ),
    "SENSOR_LOG":      BASE_DIR / "sensor_log.csv",     #传感器原始流水账单（和我记录的数据集有什么差别）
}
...
SYSTEM_CONSTANTS = {
    ...
    "SHM_REQUEST_FILE":          "/dev/shm/pred_request.json",   # Upgrade-1：改名 pred_*
    "SHM_RESPONSE_FILE":         "/dev/shm/pred_response.json",  # Upgrade-1：改名 pred_*
    "SHM_TIMEOUT_SEC":           30,        # 等待预测模型传过来的时间，超过30，会等待或者崩溃，#######需要优化####
    "PHASE2_RESPONSE_OBSERVE_LOG_INTERVAL_SEC": 1800,  # observe=0 的 Phase2 响应预测最多 30 分钟入队一次
    "PHASE2_RESPONSE_OBSERVE_HUMIDITY_DELTA": 0.3,     # observe 期间湿度变化超过 0.3% 才追加审计样本
    "PHASE2_RESPONSE_QUEUE_MAX_RECORDS": 500,          # Phase3→Phase2 响应预测队列上限，防止长期膨胀
    "PHASE2_RESPONSE_QUEUE_RETENTION_SEC": 604800,     # 响应预测队列最多保留 7 天
    "COST_SHORT_HORIZON_STEPS": 3,       # Layer5 代价函数：近端强决策窗口
    "COST_MEDIUM_HORIZON_STEPS": 6,      # Layer5 代价函数：中窗低权重风险窗口
    "COST_SHORT_HORIZON_WEIGHT": 1.0,
    "COST_MEDIUM_HORIZON_WEIGHT": 0.4,
    "COST_LONG_HORIZON_WEIGHT": 0.1,     # 第 7~12 步仅低权重审计，不再等权裁决
    ...
    "DYNAMIC_COOLDOWN_MIN_SEC":  7200,      # 动态普通冷却最短 2 小时，防止学习期高频开泵
    "DYNAMIC_COOLDOWN_MAX_SEC":  28800,     # 动态普通冷却最长 8 小时，强响应后延长观察
    "DYNAMIC_COOLDOWN_LOW_SEC":  10800,     # 靠近低湿线或响应偏弱时使用 3 小时冷却
    "DYNAMIC_COOLDOWN_MID_SEC":  14400,     # 战区中段且安全余量足够时使用 4 小时冷却
    "DYNAMIC_COOLDOWN_STRONG_DELTA": 4.0,   # 上次 3s 响应超过 4% 视为强响应，不主动缩短
    "DYNAMIC_COOLDOWN_WEAK_DELTA": 2.0,     # 上次响应低于 2% 视为弱响应，可更早复查
    "DYNAMIC_COOLDOWN_FAST_DROP_STEP": -0.12, # 最近每 5 分钟下降超过 0.12% 时缩短冷却
    "EXPLORATION_DAILY_BUDGET":  5,         # 每日最多主动阶梯探索次数（Fix-Stuck: 从2提升到5）
    ...
    "LOW_WET_RECOVERY_ENABLED":  True,       # deep drydown 后低湿恢复：减少小脉冲，生成大干大湿样本
    "LOW_WET_RECOVERY_TRIGGER_MARGIN": 0.5,  # H <= drydown_target + margin 时允许恢复脉冲绕过普通冷却
    "LOW_WET_RECOVERY_TARGET_FC_MARGIN": 5.0,# 恢复目标先到 FC-5%，不直接冲 FC
    "LOW_WET_RECOVERY_MIN_SEC":  6.0,        # 低湿恢复普通最小脉冲
    "LOW_WET_RECOVERY_MAX_SEC":  8.0,        # 低湿恢复普通单次上限
    "LOW_WET_RECOVERY_DEEP_TRIGGER": 30.0,   # H <= 30% 视为深低湿，提升恢复脉冲
    "LOW_WET_RECOVERY_DEEP_MIN_SEC": 8.0,    # 深低湿恢复至少 8s，减少 3s/4s 小补
    "LOW_WET_RECOVERY_HARD_MARGIN": 1.0,     # 距硬安全线 1% 内允许更强恢复
    "LOW_WET_RECOVERY_HARD_SEC": 10.0,       # 接近硬安全线但未进 emergency 时的恢复上限
    "LOW_WET_RECOVERY_MIN_INTERVAL_SEC": 21600,# recovery 绕过普通冷却，但同类恢复至少间隔 6h，减少开泵次数
    "LOW_WET_RECOVERY_INEFFECTIVE_REJECTS": 3, # 连续/累计多次大恢复无效后暂停，避免盲目打水
    "LOW_WET_RECOVERY_INEFFECTIVE_DELTA": 0.3, # 平均 Δm 低于 0.3% 认为大恢复没有到达探头
    "LOW_WET_RECOVERY_INEFFECTIVE_PAUSE_SEC": 21600, # 无效后暂停 6h，等待人工/视觉/水路确认
    "LOW_WET_RECOVERY_PAUSE_LOG_INTERVAL_SEC": 1800, # 暂停期重复日志最短间隔，避免每 3 分钟刷 CRITICAL
    "RESERVOIR_RETEST_ENABLED": True,          # 疑似水箱/水路无水后，允许低频自动复测是否恢复
    "RESERVOIR_RETEST_INTERVAL_SEC": 3600,     # 复测最短间隔 1h，疑似缺水后定时试探供水是否恢复
    "RESERVOIR_RETEST_SEC": 12.0,              # 5 秒仅作探测，12 秒作为服务端参考剂量复测
    "RESERVOIR_RETEST_MIN_DELTA": 0.3,         # 与传感器 0.1% 分辨率匹配，避免把微小有效响应误判为无响应
    "RESERVOIR_RETEST_HISTORY_LIMIT": 80,      # 同秒数历史比较最多回看多少条 trial
    "RESERVOIR_RETEST_SAME_SEC_TOLERANCE": 0.6,# 与复测秒数相差 <=0.6s 视为同秒数/近似同秒数
    "RESERVOIR_RETEST_MIN_BASELINE_SAMPLES": 3,# 至少 3 个同秒数样本才启用动态比较
    "RESERVOIR_RETEST_MIN_IMPROVEMENT": 0.4,   # 当前 Δm 至少高于历史中位数 0.4% 才算明显恢复
    "RESERVOIR_RETEST_IQR_FACTOR": 1.5,        # 历史波动较大时，用 median + 1.5*IQR 抬高恢复阈值
    "OBSERVE_STREAK_FORCE_THRESHOLD": 24,   # 战区连续观察 24 轮后，若安全余量足够，触发一次小探针
    "OBSERVE_STREAK_FORCE_FC_GAP": 5.0,     # 距 FC 至少 5% 才允许长期观察保护探针
    "OBSERVE_STREAK_FORCE_SEC":   3.0,      # 长期观察保护探针秒数，仍使用硬件最小安全脉冲
    ...
    "SOIL_STALE_ALERT_REPEAT_SEC": 43200,   # 同一故障邮件最短重复间隔（6 小时）
    "AIR_HUMIDITY_STALE_WARN_SEC": 21600,   # 空气湿度超过 6 小时未更新时标记为历史 fallback
    "AIR_HUMIDITY_FALLBACK_DEFAULT": 63.5,  # 没有任何空气湿度历史时的临时默认值
    "ALERT_EMAIL_TO":            "3278326875@qq.com",
    "EMERGENCY_INEFFECTIVE_MAX_COUNT": 5,     # 延长观察后再触发保护，避免短脉冲累计误报
    "EMERGENCY_INEFFECTIVE_DELTA": 0.3,       # 20 分钟结算 Δm 低于 0.3% 视为本次紧急补水无效
    "EMERGENCY_INEFFECTIVE_PAUSE_SEC": 21600, # 无效紧急补水后暂停自动 emergency 6 小时，避免水泵循环空跑
    "EMERGENCY_INEFFECTIVE_ALERT_REPEAT_SEC": 43200,
    "WATER_DELIVERY_AUTO_RECOVERY_DELTA": 2.0,  # 保护期间湿度自然/人工回升超过此值，允许一次受控复试
    "WATER_DELIVERY_AUTO_RECOVERY_TARGET_MARGIN": 1.5,  # 保护期间湿度接近 TARGET_LOW 的自动复试余量
    "EMERGENCY_INTERRUPT_COOLDOWN_SEC": 900,  # 渗透期紧急补水最短间隔（15 分钟）
    "EMERGENCY_INTERRUPT_MIN_DROP": 0.5,       # 渗透期只有继续跌破补水前湿度 0.5% 才允许二次紧急补水
...
_KNOWN_EVOLVABLE_KEYS = {
    ...
    "KP_NET_GAIN_ENABLED", "KP_NET_LOSS_SCALE", "KP_NET_MAX_LOSS",
    "DRYDOWN_STEP_SEC", "DRYDOWN_EMA_ALPHA",
    # 气候权重
    ...
    if not csv_path.exists():
        try:
            with open(csv_path, "w", encoding="utf-8") as f:
                f.write("timestamp,humidity,temperature,ec_raw,ec_norm,vpd,action_sec\n")
            logger.info(f"[ensure] 创建传感器日志: {csv_path}")
        except OSError as e:
            logger.error(f"[ensure] 创建 {csv_path} 失败: {e}")
...
class ConfigManager:
    ...
    def _ensure_runtime_defaults(self) -> None:
        """Backfill safe defaults added after an evolving_params.json already exists."""
        defaults = {
            "KP_NET_GAIN_ENABLED": True,
            "KP_NET_LOSS_SCALE": 1.0,
            "KP_NET_MAX_LOSS": 2.0,
            "DRYDOWN_STEP_SEC": 300.0,
            "DRYDOWN_EMA_ALPHA": 0.2,
        }
        missing = {key: value for key, value in defaults.items() if key not in self._params}
        if not missing:
            return
        self._params.update(missing)
        self._derive_linked_params()
        self.save()
        logger.info("[ConfigManager] 已补齐运行期新增默认参数: %s", sorted(missing))
    ...
    def _inject_phase1_genes(self) -> None:
        ...
        # Compat-1：EC_norm 安全回退——learned_ec_norm 为 null 时用 ec_buffer 均值
        if ec_norm is None:
            ec_buffer = p1_data.get("ec_buffer", [])
            if ec_buffer and len(ec_buffer) > 0:
                ec_norm = sum(ec_buffer) / len(ec_buffer)
                logger.info(
                    f"[ConfigManager] EC_norm 由 ec_buffer 均值推算: "
