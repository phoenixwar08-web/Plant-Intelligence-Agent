#!/usr/bin/env python3
"""Phase1 -> Phase3 handoff for the soil1.

Triggered by systemd timer. It checks Phase1 state once and exits.
When Phase1 has completed, it stops water-test-soil1.service, sends a pump-off
fallback to esp32/pump1/cmd, then starts phase3_soil1.service.
"""
...
STATE_PATH = Path("/root/water/phase1_test/phase1_data_soil1.json")
PHASE1_CLEAN = Path("/root/water/phase1_clean_soil1.json")
PHASE3_PARAMS = Path("/root/water/phase3/soil1/evolving_params.json")
LOCK_PATH = Path("/run/phase1_to_phase3_soil1.lock")
LOG_PATH = Path("/root/water/phase_handoff/phase1_to_phase3_soil1.log")

PHASE1_SERVICE = "water-test-soil1.service"
PHASE3_SERVICE = "phase3_soil1.service"
MQTT_BROKER = "localhost"
PUMP_TOPIC = "esp32/pump1/cmd"
...
def load_state() -> dict[str, Any]:
    if not STATE_PATH.exists():
        logger.info("Phase1 state file not found yet: %s; wait for next timer tick", STATE_PATH)
        return {}
...
def write_phase1_clean(state: dict[str, Any]) -> None:
    if PHASE1_CLEAN.exists():
        backup = PHASE1_CLEAN.with_suffix(f".json.bak_handoff_{int(time.time())}")
        PHASE1_CLEAN.rename(backup)
        logger.info("backed up old Phase1 clean data to %s", backup)
    with PHASE1_CLEAN.open("w", encoding="utf-8") as f:
        json.dump(state, f, ensure_ascii=False, indent=2)
    logger.info("wrote Phase1 clean data to %s", PHASE1_CLEAN)
