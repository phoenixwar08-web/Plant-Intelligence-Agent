...
# 路径设置
DATA_PATH = "openGauss:soil_data.soil_sensor_readings[soil_test]"
STATE_FILE = "/root/water/phase1_test/phase1_data_test.json"
MQTT_BROKER = "localhost"               
TOPIC_WATER_CMD = "esp32/pump_test/cmd"     
...
def run_control_cycle(client, current_moisture, current_temp, current_ec):
    ...
    # --- 阶段 C: 多周期调度与终极交接 ---
        ...
        if state.get("completed_cycles", 0) >= MAX_LEARNING_CYCLES:
            ...
            output_path = "/root/water/phase1_clean.json"
...
def check_csv_watchdog():
    try:
        row = _latest_soil_test_row()
        if row is None:
            print("[硬件警报] openGauss soil_sensor_readings[soil_test] 暂无有效数据，请检查 MQTT→openGauss 写入链路！")
            return False
        ...
        if idle_time_hrs > WATCHDOG_TIMEOUT_HRS:
            print(f"[硬件警报] openGauss soil_sensor_readings[soil_test] 停更 {idle_time_hrs:.1f} 小时，请检查 MQTT→openGauss 写入链路！")
            return False
        return True
    except Exception as e:
        print(f"[硬件警报] openGauss soil_sensor_readings[soil_test] 查询失败: {e}")
        return False
...
def read_latest_csv_data():
    try:
        ...
        print(f"[读取错误] openGauss soil_sensor_readings[soil_test] 查询失败: {e}")
        return None, None, None, None
