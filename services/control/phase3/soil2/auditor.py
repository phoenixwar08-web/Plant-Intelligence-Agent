...
=============================================================================
 ...
 【数据来源】
   · GaussDB soil_sensor_readings：土壤传感器时序与空气湿度
   · GaussDB irrigation_events：浇水事件与持续时间
 ...
 【边缘场景修复记录】：
   ...
   Fix-E6  历史读取统一走 GaussDB，避免本地文件并发与损坏风险。
...
DEVICE_CODE = "soil2"
PARAMETER_CHANGE_SYSTEM = "phase3_soil2"
...
def _gsql_query(sql: str) -> str:
    result = subprocess.run(
        ["su", "-", "opengauss", "-c", "gsql -d soil_data -p 7654 -t -A -F ','"],
        input=sql,
        capture_output=True,
        text=True,
        timeout=30,
...
def _load_sensor_history(hours: int = 24) -> list[dict]:
    """从 GaussDB 读取最近 hours 小时的有效传感器和浇水事件记录。"""
    cutoff = datetime.now() - timedelta(hours=hours)
    records: list[dict] = []
    skipped_bad_quality = 0
    try:
        sql = (
            "SELECT s.recv_time, s.humidity, s.temp, s.ec, "
            "COALESCE(s.air_humidity, (SELECT prior.air_humidity FROM soil_sensor_readings prior "
            "WHERE prior.device_code='soil2' AND prior.air_humidity IS NOT NULL "
            "AND prior.recv_time <= s.recv_time ORDER BY prior.recv_time DESC, prior.id DESC LIMIT 1)), "
            "COALESCE((SELECT max(e.water_sec) FROM irrigation_events e "
            "WHERE e.device_code='soil2' AND e.command_time >= s.recv_time "
            "AND e.command_time <= s.recv_time + INTERVAL '5 minutes'), 0) "
            "FROM soil_sensor_readings s "
            "WHERE s.device_code='soil2' AND s.recv_time >= "
            + _sql_literal(cutoff.strftime("%Y-%m-%d %H:%M:%S"))
            + " AND s.humidity IS NOT NULL AND s.temp IS NOT NULL AND s.ec IS NOT NULL "
            "ORDER BY s.recv_time ASC, s.id ASC;"
        )
        rows = _gsql_query(sql).splitlines()
    except (OSError, RuntimeError, subprocess.SubprocessError) as e:
        logger.error(f"[IO] 读取 GaussDB 传感器历史失败: {e}")
        return records

    for row in rows:
        try:
            recv_time, humidity_raw, temperature_raw, ec_raw, air_rh_raw, action_raw = row.split(",", 5)
            ts = datetime.strptime(recv_time, "%Y-%m-%d %H:%M:%S")
            humidity = float(humidity_raw)
            temperature = float(temperature_raw)
            ec_norm = float(ec_raw) / max(1.0 + 0.02 * (temperature - 25.0), 0.1)
            air_rh = float(air_rh_raw)
            es = 0.6108 * math.exp(17.27 * temperature / (temperature + 237.3))
            vpd = min(max(es * (1.0 - air_rh / 100.0), 0.0), _VPD_MAX_KPA)
            action_sec = float(action_raw)
    ...
    logger.info(
        f"[IO] 从 GaussDB 加载过去 {hours}h: {len(records)} 条"
...
class NightlyAuditor:
    ...
    def run_full_audit(self) -> None:
        ...
        df_26h = _load_sensor_history(hours=26)   # Fix-E1：扩大到 26h，覆盖深夜大灌溉的观察期
        df_7d  = _load_sensor_history(hours=24 * 7)
