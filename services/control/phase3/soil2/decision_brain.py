...
=============================================================================
 ...
 【对齐修改记录】（对应 config_manager.py 的修复）：
   ...
   Align-7  删除旧版中对不存在的 SYSTEM_PARAMS /
   ...
   Align-10 运行状态写入统一使用受锁保护的 JSON 文件。
...
# 与 phase1_test/water-test-soil2.py 对齐；可用环境变量覆盖
MQTT_BROKER_DEFAULT = os.environ.get("IRRIGATION_MQTT_BROKER", "localhost")
MQTT_TOPIC_PUMP_CMD_DEFAULT = os.environ.get(
    "IRRIGATION_MQTT_TOPIC_PUMP", "esp32/pump2/cmd"
)
DEVICE_CODE_DEFAULT = "soil2"
...
def _gsql_exec(sql: str) -> None:
    sql_path = Path(f"/tmp/{DEVICE_CODE_DEFAULT}_gsql_{uuid.uuid4().hex}.sql")
...
def _send_alert_email(subject: str, body: str) -> bool:
    ...
    from_addr = os.environ.get("IRRIGATION_ALERT_FROM", username or f"soil2@{os.uname().nodename}")
...
class SensorLayer:
    ...
    数据来源：
      · openGauss soil_data.soil_sensor_readings[soil2]
            每 ~5 分钟一行：土壤湿度(%)、温度(°C)、电导率、空气湿度 等
    VPD：使用 GaussDB air_humidity 与本轮土壤温度计算。
    """
    ...
    def __init__(self, cfg: ConfigManager):
        self.cfg = cfg
        # 缓存上一次成功读取的 GaussDB 空气湿度，用于数据库短暂不可用时保守降级。
        self._last_air_rh:   float = 60.0   # 初始保守默认值
        self._last_air_temp: float = 25.0
        self._last_gauss_air_rh: Optional[float] = None
        self._last_gauss_air_ts: Optional[float] = None
    ...
    def _read_soil_raw(self) -> tuple[float, float, float, float, int]:
        """从 openGauss 统一传感器表读取 soil2 最新一条有效土壤数据。"""
        sql = (
            "SELECT id, recv_time, temp, humidity, ec, air_humidity FROM soil_sensor_readings "
            "WHERE device_code='soil2' "
        ...
        try:
            ...
            if not out:
                raise RuntimeError("openGauss soil_sensor_readings[soil2] 表中没有有效土壤记录")
            parts = out.split(",")
            if len(parts) < 5:
                raise RuntimeError(f"openGauss 返回格式异常: {out}")
            _row_id, recv_time, temp, humidity, ec_raw = parts[:5]
            air_rh_raw = parts[5].strip() if len(parts) > 5 else ""
            ...
            if not (
                ...
                _update_system_state(update)
                raise RuntimeError(
                    f"openGauss soil_sensor_readings[soil2] 最新有效记录超出物理范围: "
                    f"H={humidity_f}, T={temp_f}, EC={ec_f}"
                )
            soil_ts = self._parse_soil_timestamp(recv_time)
            self._remember_gauss_air_humidity(air_rh_raw, soil_ts)
            return humidity_f, temp_f, ec_f, soil_ts, 0
        except (ValueError, OSError, subprocess.SubprocessError) as e:
            raise RuntimeError(f"openGauss soil_sensor_readings[soil2] 读取失败: {e}") from e

    def _remember_gauss_air_humidity(self, raw_rh: Any, ts: Optional[float]) -> None:
        if raw_rh in (None, ""):
            return
        try:
            air_rh = float(raw_rh)
        except (TypeError, ValueError):
            logger.warning(f"[Layer0] GaussDB air_humidity 解析失败: {raw_rh!r}")
            return
        if not (0.0 <= air_rh <= 100.0):
            logger.warning(f"[Layer0] GaussDB air_humidity 超出范围: {air_rh}%")
            return
        self._last_gauss_air_rh = air_rh
        self._last_gauss_air_ts = ts
    ...
    def _recent_humidity_slope(self) -> Optional[float]:
        """Return the latest six GaussDB readings' humidity slope in % per sample."""
        sql = (
            "SELECT humidity FROM ("
            "SELECT humidity, recv_time, id FROM soil_sensor_readings "
            "WHERE device_code='soil2' AND humidity IS NOT NULL "
            "ORDER BY recv_time DESC, id DESC LIMIT 6"
            ") recent ORDER BY recv_time ASC, id ASC;"
        )
        cmd = "gsql -d soil_data -p 7654 -t -A -c " + repr(sql)
        try:
            result = subprocess.run(
                ["su", "-", "opengauss", "-c", cmd],
                capture_output=True, text=True, timeout=30,
            ...
            if result.returncode != 0:
                raise RuntimeError(result.stderr.strip() or result.stdout.strip())
            series = [float(value) for value in result.stdout.split() if value.strip()]
            if len(series) < 6:
                return None
            ...
            logger.warning(f"[Layer0] GaussDB recent humidity slope unavailable: {exc}")
            return None
    ...
    def _maybe_alert_stale_sensor(
        ...
        if now - last_sent < repeat_sec:
            return
        subject = "soil2 报警：土壤传感器数据超时"
        body = (
            f"智养中心检测到 soil2 土壤传感器数据异常。\n\n"
            f"报警类型：土壤传感器有效数据超时\n"
            ...
            f"建议操作：检查 soil2 传感器供电、接线、ESP 上报和 MQTT/CSV 数据链路。"
        )
    ...
    def _read_air_raw(self, soil_temp_for_vpd: float) -> tuple[float, float]:
        ...
        空气湿度异常时不应阻塞 phase3。容错顺序：
          ...
          2. 当前进程缓存的最后有效 GaussDB 值；
          3. 保守默认值 AIR_HUMIDITY_FALLBACK_DEFAULT。
        ...
        if gauss_air is not None:
            ...
            self._last_air_temp = air_temp
            self._last_gauss_air_rh = last_rh
            self._last_gauss_air_ts = air_ts
        ...
        return use_fallback("cache_or_default", "gaussdb_unavailable")
    ...
    def _read_latest_gauss_air_humidity(self) -> Optional[tuple[float, float]]:
        sql = (
            ...
            "WHERE device_code='soil2' AND air_humidity IS NOT NULL "
            "ORDER BY recv_time DESC, id DESC LIMIT 1;"
        )
        cmd = "gsql -d soil_data -p 7654 -t -A -F \",\" -c " + repr(sql)
        try:
            ...
            logger.warning(f"[Layer0] GaussDB air_humidity 读取失败，使用进程缓存或保守默认值: {e}")
            return None
...
class PhysicsLayer:
    ...
    def calc_water_ceiling(self, current_humidity: float) -> float:
        ...
        kp, guard = _guarded_zone_kp(self.cfg, zone_name)
        ...
        if guard:
            logger.info(
                "[ResponseGuard] ceiling uses recent strong response: "
                f"zone={zone_name} dps={float(guard['delta_per_sec']):.3f}%/s "
    ...
    def _guard_hard_safety_low(
        ...
        override_raw = self.cfg.get_constant("HARD_SAFETY_LOW_OVERRIDE")
        if override_raw is not None:
            try:
                effective = round(float(override_raw), 3)
            except (TypeError, ValueError):
                logger.warning(f"[HardSafety] invalid HARD_SAFETY_LOW_OVERRIDE={override_raw!r}, fallback to dynamic line.")
            else:
                ...
                def update(current):
                    current["hard_safety_low_guard"] = {
                        ...
                        "last_lowered_at": guard.get("last_lowered_at"),
                        "updated_at": now,
                    }
                ...
                try:
                    ...
                    logger.warning(f"[HardSafety] guard override state update failed: {e}")

                logger.warning(
                    f"[HardSafety] fixed override active: derived={derived_line:.3f}% "
...
class GateLayer:
    ...
    def _allowed_exploration_sec(
        ...
        if guard:
            ...
            safe_line = min(float(self.cfg.FC) - 0.2, float(self.cfg.M_SAFE_SLEEP) - 0.2)
            projected_min_peak = float(reading.humidity) + guard_kp * water_min
            if projected_min_peak > safe_line:
                max_allowed = 0.0
                reason += (
                    f", response_guard_veto({projected_min_peak:.1f}>{safe_line:.1f},"
                    f"dps={guard_kp:.3f})"
                )
...
class Phase2Predictor:
        ...
        while time.monotonic() < deadline:
            if self.resp_path.exists():
                try:
                    ...
                    if bool(self.cfg.get_constant("PHASE2_SHADOW_ONLY")):
                        self._record_shadow_response(resp, plans, reading)
                        logger.info("[Layer4] Phase2 影子模式：预测已记录，控泵仍使用物理外推。")
                        return self._fallback_trajectories(plans, reading)
                    return trajectories
    ...
    def _record_shadow_response(self, response: dict, plans: list[ActionPlan], reading: SensorReading) -> None:
        def update(state):
            state["phase2_shadow_last"] = {
                "updated_at": time.time(),
                "request_id": response.get("request_id"),
                "sensor_humidity": round(float(reading.humidity), 4),
                "candidate_metrics": response.get("candidate_metrics", {}),
                "trajectory_ends": {
                    label: values[-1]
                    for label, values in (response.get("trajectories") or {}).items()
                    if isinstance(values, list) and values
                },
                "plans": [{"label": p.label, "water_sec": p.water_sec} for p in plans],
                "control_effect": "shadow_only_physical_fallback_used",
            }
        try:
            _update_system_state(update)
        except Exception as exc:
            logger.warning("[Layer4] Phase2 影子结果写入 system_state 失败: %s", exc)
...
class ActuatorLayer:
    ...
    Fix-A：渗透等待由 PendingSoak 管理，不再用 1800s 阻塞主线程。
           MQTT 浇水段与 water-test-soil2.py 一致：on → sleep(sec) → off，
           该段会阻塞当前线程。
    """
...
class DecisionBrain:
    ...
    def _record_soak_observation(self, soak: PendingSoak, reading: SensorReading) -> None:
        ...
        previous_peak = float(getattr(soak, "observed_peak", soak.pre_humidity) or soak.pre_humidity)
        observed_peak = max(previous_peak, float(reading.humidity))
        changed = observed_peak != previous_peak
        ...
        if changed:
            def update(state):
                if "pending_soak" in state:
                    ...
                    state["pending_soak"]["observed_peak"] = observed_peak
            _update_system_state(update)
    ...
    def _style_experiment(self, reading: SensorReading) -> dict[str, Any]:
        ...
        style_mode = str(
            self.cfg.get_constant("IRRIGATION_STYLE_MODE") or "adaptive"
        ).strip().lower()
        ...
        if style_mode in {"see_dry_wet", "dry_wet", "jian_gan_jian_shi"}:
            deep_enabled = False
            vpd_target_reason = "see_dry_wet"
            target = float(self.cfg.TARGET_LOW)
        elif deep_enabled:
        ...
        if not isinstance(exp, dict) or "mode" not in exp or "started_at" not in exp:
            ...
            target_low_crossed = (
                style_mode in {"see_dry_wet", "dry_wet", "jian_gan_jian_shi"}
                and float(reading.humidity) <= float(self.cfg.TARGET_LOW)
            )
            if mode == "drydown_cycle" and (
                deep_drydown_time_complete or reading.humidity <= target or target_low_crossed
            ):
                exp = {
                    "enabled": True,
                    "mode": "wet_hold",
                    ...
                    "drydown_target": round(target, 3),
                    "reason": (
                        "target_low_crossed_see_dry_wet"
                        if target_low_crossed else "drydown_window_complete"
                    ),
        ...
        if style_mode in {"see_dry_wet", "dry_wet", "jian_gan_jian_shi"} and reading.humidity <= self.cfg.TARGET_LOW:
            force_observe = False
        exp["drydown_target"] = round(target, 3)
        exp["style_mode"] = style_mode
