...
class PredictorCircuitBreaker:
    ...
    def before_cycle(self) -> None:
        ...
        · HALF_OPEN → 每轮都确保 PREDICTOR_CIRCUIT_OPEN=False，等待下一次真实预测穿透
        · CLOSED → 确保 PREDICTOR_CIRCUIT_OPEN=False，避免重启后残留熔断标志
        """
        now = time.monotonic()
        ...
        elif self._state in (self._STATE_HALF_OPEN, self._STATE_CLOSED):
            # 服务重启后可能恢复为 HALF_OPEN，但 evolving_params 里仍残留
            # PREDICTOR_CIRCUIT_OPEN=True；CLOSED 也可能遇到同样的磁盘残留。
            # 只要不是 OPEN，就不应阻断 Phase2。
            if self._cfg.get("PREDICTOR_CIRCUIT_OPEN", False):
                logger.info(f"[CB] {self._state} 状态恢复预测通道，允许 Phase2 请求。")
            self._cfg.update({"PREDICTOR_CIRCUIT_OPEN": False}, persist=False)
...
def main() -> None:
    ...
    # ── Step 0：等待系统时间可信 ─────────────────────────────────────
    # 必须早于 system_state / ConfigManager / 熔断器恢复，避免 1970 时间污染
    # 冷却时间、熔断恢复和经验记录。
    _wait_for_valid_wall_clock()

    # ── Step 1：初始化依赖文件 ─────────────────────────────────────────
    # 创建 system_state.json / pattern_memory.json / sensor_log.csv（若不存在）
