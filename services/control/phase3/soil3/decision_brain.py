...
# Cross-plant experience is optional metadata.  A missing or broken bridge
# must never prevent Phase3 from running its existing safety controller.
try:
    from experience_validation import ExperienceValidationBridge
except Exception:  # pragma: no cover - deployment fallback for Phase3
    ExperienceValidationBridge = None  # type: ignore[assignment,misc]
...
class DecisionBrain:
    ...
    def __init__(self, cfg: ConfigManager):
        ...
        self._experience_validation = (
            ExperienceValidationBridge() if ExperienceValidationBridge is not None else None
        )
