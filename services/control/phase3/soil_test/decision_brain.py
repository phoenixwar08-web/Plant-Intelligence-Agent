...
# 与 phase1_test/water_test.py 对齐；可用环境变量覆盖
MQTT_BROKER_DEFAULT = os.environ.get("IRRIGATION_MQTT_BROKER", "localhost")
MQTT_TOPIC_PUMP_CMD_DEFAULT = os.environ.get(
    "IRRIGATION_MQTT_TOPIC_PUMP", "esp32/pump_test/cmd"
)
DEVICE_CODE_DEFAULT = "soil_test"
...
def _send_alert_email(subject: str, body: str) -> bool:
    ...
    from_addr = os.environ.get("IRRIGATION_ALERT_FROM", username or f"soil_test@{os.uname().nodename}")
...
class SensorLayer:
    ...
    数据来源：
      · openGauss soil_data.soil_sensor_readings[soil_test]
            每 ~5 分钟一行：土壤湿度(%)、温度(°C)、电导率、空气湿度 等
      · air_humidity.csv —— 仅作为 GaussDB 空气湿度异常时的兼容 fallback

    VPD：使用空气湿度 RH_air（优先来自 GaussDB air_humidity）与气温 T（与 soil_test 脚本一致，取土壤最新行「温度(°C)」，
    ...
    def read(self, require_fresh: bool = False) -> SensorReading:
        ...
        # ── 空气湿度来自 air CSV；VPD 用气温 = 与 water_test 同源（土壤 CSV「温度(°C)」）
    ...
    def _read_soil_raw(self) -> tuple[float, float, float, float, int]:
        ...
        try:
            ...
            if not out:
                raise RuntimeError("openGauss soil_sensor_readings[soil_test] 表中没有有效土壤记录")
            parts = out.split(",")
            if len(parts) < 5:
            ...
            if not (
                ...
                _update_system_state(update)
                raise RuntimeError(
                    f"openGauss soil_sensor_readings[soil_test] 最新有效记录超出物理范围: "
            ...
            raise RuntimeError(f"openGauss soil_sensor_readings[soil_test] 读取失败: {e}") from e
    ...
    def _maybe_alert_stale_sensor(
        ...
        if now - last_sent < repeat_sec:
            return
        subject = "soil_test 报警：土壤传感器数据超时"
        body = (
            f"智养中心检测到 soil_test 土壤传感器数据异常。\n\n"
            f"报警类型：土壤传感器有效数据超时\n"
            ...
            f"建议操作：检查 soil_test 传感器供电、接线、ESP 上报和 MQTT/CSV 数据链路。"
        )
    ...
    def _read_latest_gauss_air_humidity(self) -> Optional[tuple[float, float]]:
        sql = (
            ...
            "WHERE device_code='soil_test' AND air_humidity IS NOT NULL "
            "ORDER BY recv_time DESC, id DESC LIMIT 1;"
        )
        cmd = "gsql -d soil_data -p 7654 -t -A -F \",\" -c " + repr(sql)
        try:
...
class ActuatorLayer:
    ...
    Fix-A：渗透等待由 PendingSoak 管理，不再用 1800s 阻塞主线程。
           MQTT 浇水段与 water_test.py 一致：on → sleep(sec) → off，
           该段会阻塞当前线程。
    """
