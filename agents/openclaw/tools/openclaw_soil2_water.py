...
DEVICE_CODE = "soil2"
PUMP_TOPIC = "esp32/pump2/cmd"
PUMP_META_TOPIC = "esp32/pump2/cmd_meta"
MQTT_HOST = "localhost"
STATE_PATH = Path("/root/water/phase3/soil2/system_state.json")
AUDIT_PATH = Path("/root/.openclaw/workspace/logs/openclaw_soil2_irrigation_requests.jsonl")
LOCK_PATH = Path("/tmp/openclaw_soil2_water.lock")

MAX_USER_SEC = 10.0
TARGET_LOW_FALLBACK = 30.0
...
def latest_irrigation_age_sec() -> float | None:
    rows = gsql_scalar_rows(
        ...
        "WHERE device_code='soil2' AND water_sec IS NOT NULL AND water_sec > 0 "
...
def main() -> None:
    parser = argparse.ArgumentParser(description="OpenClaw guarded soil2 watering gateway")
    parser.add_argument("--seconds", type=float, required=True)
    parser.add_argument("--phase3-recommended-seconds", type=float)
    parser.add_argument("--operator-id", default="openclaw-qqbot4")
    ...
    with LOCK_PATH.open("w", encoding="utf-8") as lock_fh:
        ...
        if context.get("soft_risk_overridden"):
            recommendation = args.phase3_recommended_seconds
            if recommendation is None or recommendation <= 0:
                reject(
                    request,
                    "risk_override_without_phase3_recommendation",
                    "风险确认时必须使用当前自动养护给出的剂量，不能凭空指定浇水时长。",
                    context,
                )
            if args.seconds > recommendation:
                reject(
                    request,
                    "risk_override_seconds_too_high",
                    f"风险确认时不能超过当前自动养护建议的 {recommendation:g} 秒。",
                    context,
                )
