...
capture_failed means RTSP did not provide a decodable image or the zone could not be cut out of it. Its failure record contains attempt ID, timestamp, and a sanitised error code, plus plant_zone when a zone was reached. analysis_failed means the capture, crop, and storage worked but the model timed out, the request failed, invalid JSON came back, or semantic validation rejected it; its failure record additionally contains image_id, image_sha256, plant_zone, source_frame_id, http_status, and provider_error_code. Neither failure status may be relabelled as image quality or fabricate visual facts.

The two provider fields answer what the service itself could not tell apart during the first quota block. http_status is the status code of the reply that was received, so 401, 403, 429, and 5xx are distinguishable; provider_error_code is one bounded machine token read from that reply's error field. Both are null when no reply arrived at all (timeout, connection failure) and when only local validation rejected an answer, so an absent provider response is never recorded as if the provider had refused. The fields report the transport truth, not an inference: measured against the real endpoint, a rejected small request answers 401 with error code invalid_api_key, while the same rejection carrying a zone-sized image body closes the connection before any status line, so that attempt is recorded as a request failure with two nulls. Only those two values are extracted: the response body, its message text, the API key, the authorization header, the endpoint URL, the Base64 image, the prompt, and the raw model answer are never stored, and the underlying requests exception is not chained, because its own text carries the URL. The local error_code stays the stable classification consumers match on.
...
Tests use fake capture and analyzer dependencies, a fake HTTP session, temporary data directories, and no real RTSP or Qwen request. They cover all four per-zone outcomes and failure isolation, whole-frame versus crop storage and hashing, rect-to-pixel mapping across resolutions, invalid JSON, every semantic rule above, the prompt staying aligned with the contract, per-zone history: the earlier image offered to a zone is that zone's own most recent readable crop, never the other plant's, and provider diagnostics: a fake session returns 400, 401, 403, 429, 5xx, an unparseable error body, a timeout, and a connection failure, and each case must be diagnosable from the two fields alone while no message text, URL, credential, or image bytes may appear in the stored failure or the exception text. A real camera and model smoke test is manual, requires explicit user approval, and its zone rectangles are checked by eye against the saved crops.
