...
For each Review batch, the Owner checks conflicts, integration behavior, tests, scope, and Phase3/MQTT/production-data boundaries before manually merging approved PRs. GitHub Issues, rather than this document, define task order, ownership, and acceptance timing.

## Review handoff

Every implementation remains in **Review** after its own tests pass. The handoff must state: (1) what changed, (2) what was not changed, (3) affected modules, (4) actual verification, and (5) remaining issues or risks. The Owner decides Done only after checking behavior, tests, diff scope, protocol compatibility, the relevant integration batch, and Phase3/MQTT/production-data boundaries.
